...
/****** Object:  StoredProcedure [bak].[sp_DatabaseRestore]    Script Date: 6/12/2017 11:57:59 AM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[bak].[sp_DatabaseRestore]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [bak].[sp_DatabaseRestore] AS' 
END
GO


ALTER PROCEDURE [bak].[sp_DatabaseRestore]
...
BEGIN
	...
	PRINT @sql;
	IF @Debug = 0
		EXECUTE @sql = [ola].[CommandExecute] @Command = @sql, @CommandType = 'RESTORE DATABASE', @Mode = 1, @DatabaseName = @Database, @LogToTable = 'Y', @Execute = 'Y';
...
BEGIN
	...
	EXECUTE (@sql);
		...
		IF (@ContinueLogs = 0 AND @LogFirstLSN <= @FullLastLSN AND @FullLastLSN <= @LogLastLSN AND @RestoreDiff = 0) OR (@ContinueLogs = 1 AND @LogFirstLSN <= @DatabaseLastLSN AND @DatabaseLastLSN < @LogLastLSN AND @RestoreDiff = 0)
			SET @i = 2;
		IF (@ContinueLogs = 0 AND @LogFirstLSN <= @DiffLastLSN AND @DiffLastLSN <= @LogLastLSN AND @RestoreDiff = 1) OR (@ContinueLogs = 1 AND @LogFirstLSN <= @DatabaseLastLSN AND @DatabaseLastLSN < @LogLastLSN AND @RestoreDiff = 1) OR (@ContinueLogs = 0 AND @LogFirstLSN <= @FullLastLSN AND @FullLastLSN <= @LogLastLSN AND @RestoreDiff = 1) OR (@ContinueLogs = 1 AND @LogFirstLSN <= @DatabaseLastLSN AND @DatabaseLastLSN < @LogLastLSN AND @RestoreDiff = 1)
			SET @i = 2;
	...
	BEGIN
		...
		PRINT @sql
		IF @Debug = 0
			EXECUTE @sql = [ola].[CommandExecute] @Command = @sql, @CommandType = 'RESTORE LOG', @Mode = 1, @DatabaseName = @Database, @LogToTable = 'Y', @Execute = 'Y';
	END;
	FETCH NEXT FROM BackupFiles INTO @BackupFile;
END;
CLOSE BackupFiles;
...
IF @RunCheckDB = 1
BEGIN
	SET @sql = 'EXECUTE [ola].[DatabaseIntegrityCheck] @Databases = ' + @RestoreDatabaseName + ', @LogToTable = ''Y'''+CHAR(13);
	PRINT @sql
	IF @Debug = 0
